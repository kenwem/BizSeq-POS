...
const SuperAdmin: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const storeData = storesSnap.docs.map(
        (doc) =>
          ({
            id: doc.id,
            ...doc.data(),
  ...
  const handleOnboardStore = useCallback(
    async (e?: React.FormEvent) => {
      ...
      try {
        ...
        const payload = {
          ...
          adminUsername:
            newStore.adminUsername || newStore.adminEmail.split("@")[0],
        ...
        if (!response.ok || resData.success === false) {
          throw new Error(
            resData.error || resData.message || `HTTP ${response.status}`,
          );
        }
        ...
        try {
          ...
          const matchedDoc = querySnap.docs.find((doc) => {
            const d = doc.data();
            return (
              d.name === newStore.storeName.trim() ||
              (d.adminEmail && d.adminEmail === newStore.adminEmail.trim())
            );
          });
  ...
  const handleSaveStoreUpdates = useCallback(
    async (e: React.FormEvent) => {
      e.preventDefault();
  ...
  const handleEditStoreName = useCallback(
    async (storeId: string, currentName: string) => {
      const newName = prompt(
        `Enter new store display name (leaving blank or matching cancels):`,
        currentName,
      );
  ...
  const handleManageUser = useCallback(
    async (uid: string, action: string, data?: any) => {
      try {
        ...
        if (!response.ok || !resData.success) {
          throw new Error(
            resData.error ||
              resData.message ||
              `Management error: ${response.status}`,
          );
        }
        fetchData();
        return true;
  ...
  const handleRepairUser = useCallback(
    async (e: React.FormEvent) => {
      e.preventDefault();
  ...
  const handleManualCreateStore = useCallback(
    async (e: React.FormEvent) => {
      e.preventDefault();
      ...
      setIsRepairing(true);
      try {
        const storeIdDoc = manualStoreId
          .trim()
          .toLowerCase()
          .replace(/[^a-z0-9_-]/g, "_");
        ...
        await setDoc(storeDocRef, {
          ...
          adminEmail:
            manualStoreAdminEmail.trim() || "unassigned@bizseq.internal",
      ...
      <div className="min-h-screen flex items-center justify-center bg-white p-8 font-sans">
        <div className="max-w-md w-full bg-slate-50 p-12 rounded-[3.5rem] border-4 border-slate-100 text-center">
          ...
          <p className="text-slate-600 font-bold italic mb-8">
            Access restricted to established Super Admin accounts only. (UID:{" "}
        ...
        <div>
          ...
          >
            ...
            <span className="text-[11px] font-black uppercase tracking-[0.4em] text-emerald-600">
              Global Admin Terminal
            </span>
          </motion.div>
          <h1 className="text-6xl md:text-7xl font-black tracking-tight text-slate-900 leading-none mb-6 italic">
            System Console
          </h1>
          ...
          <p className="text-slate-600 font-bold text-xl max-w-xl italic leading-relaxed">
            Provision stores and configure administrative credentials for the{" "}
            <span className="text-slate-950 underline decoration-emerald-500 decoration-4 underline-offset-4">
              BizSeq Ecosystem
            </span>
            .
          </p>
        ...
        {[
          {
            ...
            desc: "Define store name and admin email.",
            icon: Building2,
          },
          {
            step: "02",
            title: "Auto-Create",
            desc: "System automatically provisions the user account.",
            icon: Zap,
          },
          {
            step: "03",
            title: "Login Share",
            desc: "Securely share login credentials with the store owner.",
            icon: Globe,
          },
          {
            step: "04",
            title: "Deployment",
            desc: "Full store access activated instantly.",
            icon: CheckCircle2,
          },
        ].map((g, i) => (
          <div
            key={i}
            className="bg-slate-900 p-8 rounded-[2.5rem] border border-slate-800 relative overflow-hidden group"
          >
            <span className="absolute -right-4 -bottom-4 text-6xl font-black text-white/5 group-hover:text-emerald-500/10 transition-colors">
              {g.step}
            </span>
            <div className="p-3 bg-slate-800 rounded-xl w-fit mb-4">
              <g.icon className="h-5 w-5 text-emerald-400" />
            </div>
            <h4 className="text-white font-black text-[10px] uppercase tracking-[0.2em] mb-2">
              {g.title}
            </h4>
            <p className="text-slate-500 text-[10px] font-bold italic leading-relaxed">
              {g.desc}
            </p>
          </div>
        ))}
            ...
            {loading ? (
              Array(2)
                ...
                .map((_, i) => (
                  <div
                    key={i}
                    className="h-80 bg-white/50 rounded-[3rem] animate-pulse"
                  />
                ))
            ) : stores.length === 0 ? (
                ...
                >
                  <div className="flex items-start justify-between mb-8">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <h3 className="font-black text-xl text-slate-950 tracking-tight uppercase">
                          {store.name || (store as any).storeName || "N/A"}
                        </h3>
                  ...
                  <div className="grid grid-cols-2 gap-6 py-6 border-y border-slate-100">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-black text-slate-700 font-mono tracking-tight truncate">
                        {store.id}
                      </p>
                    </div>
                  </div>
                    ...
                    <button
                      onClick={async () => {
                        const confirmAction = confirm(
                          `Toggle status for ${store.name}? (Active <-> Suspended)`,
                        );
                        if (confirmAction) {
                          setLoading(true);
                          try {
          ...
          <div className="bg-white p-8 rounded-[3rem] border-2 border-slate-100 shadow-xl space-y-6">
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
              {filteredUsers.map((user) => {
                ...
                return (
                  <div
                    key={user.id || `user-admin-${user.username}`}
                    className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 group hover:bg-white hover:shadow-lg transition-all"
                  >
                    ...
                    <div className="mb-4 pt-3 border-t border-slate-100/60 grid grid-cols-2 gap-4 text-[9px] font-bold text-slate-500">
                      <div>
                        <span className="text-[8.5px] text-slate-400 font-black uppercase tracking-widest block mb-0.5">
                          Store Hub
                        </span>
                        <span className="text-slate-800 font-black uppercase truncate block">
                          {assignedStoreName}
                        </span>
                      </div>
                      <div>
                        ...
                        <span
                          className="font-mono text-slate-800 text-[8px] truncate block"
                          title={user.id}
                        >
                          {user.id}
                        </span>
                    ...
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button
                        onClick={async () => {
                          if (
                            confirm(`Delete user ${user.username} permanently?`)
                          ) {
                            try {
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="mb-8 text-center shrink-0">
                ...
                <h2 className="text-3xl md:text-4xl font-black text-slate-950 tracking-tight uppercase italic">
                  Store Onboarding
                </h2>
                <p className="text-slate-600 font-bold italic mt-1 text-sm">
                  Provision a new franchise entry.
                </p>
              </div>

              <form
                onSubmit={handleOnboardStore}
                className="space-y-6 overflow-y-auto pr-4 custom-scrollbar"
              >
                <div className="group">
                  <label className="block text-[11px] font-black text-slate-900 uppercase tracking-widest mb-3 ml-2 group-focus-within:text-emerald-600 transition-colors">
                    Store Name
                  </label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="group flex flex-col font-sans">
                    <label className="block text-[11px] font-black text-slate-900 uppercase tracking-widest mb-3 ml-2">
                      Store Address
                    </label>
                    <input
                      type="text"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="group flex flex-col">
                    <label className="block text-[11px] font-black text-slate-800 uppercase tracking-widest mb-3 ml-2 group-focus-within:text-emerald-600 transition-colors">
                      Admin Email
                    </label>
                    <input
                      type="email"
                      ...
                      onChange={(e) =>
                        setNewStore({ ...newStore, adminEmail: e.target.value })
                      }
                ...
                <div className="group flex flex-col relative">
                  ...
                  <p className="mt-3 text-[9px] text-slate-600 font-bold italic px-2">
                    The admin will use this to login for the first time.
                  </p>
                </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="mb-8 text-center shrink-0">
                ...
                <h2 className="text-3xl md:text-4xl font-black text-slate-950 tracking-tight uppercase italic animate-fade-in">
                  Manage Configuration
                </h2>
                <p className="text-slate-600 font-bold italic mt-1 text-sm">
                  Update metadata for franchise node: {editingStore.id}
                </p>
              </div>

              <form
                onSubmit={handleSaveStoreUpdates}
                className="space-y-6 overflow-y-auto pr-4 custom-scrollbar"
              >
                <div className="group">
                  <label className="block text-[11px] font-black text-slate-900 uppercase tracking-widest mb-3 ml-2 group-focus-within:text-emerald-600 transition-colors">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            ...
            <p className="text-slate-500 font-bold text-sm italic mt-1">
              Directly look up database records and bypass Cloud Function lag to
              correct user accounts or synchronize stores.
            </p>
          </div>
          <button
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-slate-900 text-white p-10 rounded-[2.5rem] border-2 border-slate-800 shadow-2xl relative">
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-slate-400 font-bold text-xs italic">
                        Force-map any login (e.g. kerm@yahoo.com) to a custom
                        role & store
                      </p>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleRepairUser} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        >
                          ...
                          {stores.map((st, i) => (
                            <option
                              key={st.id || `st-option-${i}`}
                              value={st.id}
                            >
                              {st.name} ({(st.id || "").substring(0, 8)})
                            </option>
                          ))}
                        </select>
                      </div>
                    ...
                    <div className="pt-2">
                      ...
                      >
                        {isRepairing
                          ? "Repairing Firestore Node..."
                          : "Force-Map User Credentials"}
                      </button>
                    </div>
                  </form>
                </div>
                ...
                <div className="bg-slate-900 text-white p-10 rounded-[2.5rem] border-2 border-slate-800 shadow-2xl relative">
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <h4 className="font-black text-xl tracking-tight uppercase">
                        Direct Store Creator
                      </h4>
                      <p className="text-slate-400 font-bold text-xs italic">
                        Instantiate a store into the Stores collection directly
                      </p>
                    </div>
                  </div>

                  <form
                    onSubmit={handleManualCreateStore}
                    className="space-y-4"
                  >
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">
                          Store ID (Letters/No Spaces)
                        </label>
                        <input
                          type="text"
                          required
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            setManualStoreAdminEmail(e.target.value);
                          }}
                          placeholder="kenny@example.com"
                          className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-xs outline-none focus:border-blue-500 text-white font-bold placeholder:text-slate-500"
                        />
                      ...
                      <div>
                        <label className="block text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">
                          Admin User Account UID
                        </label>
                        <input
                          type="text"
                          value={manualStoreAdminId}
                          onChange={(e) =>
                            setManualStoreAdminId(e.target.value)
                          }
              ...
              <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                    <p className="font-extrabold text-[10px] text-slate-400 uppercase tracking-widest mb-4">
                      Stores Registries ({stores.length})
                    </p>
                    <div className="max-h-60 overflow-y-auto space-y-2 pr-1 custom-scrollbar text-[10px]">
                      {stores.map((st) => (
                        <div
                          key={st.id || `st-${st.name}`}
                          className="flex justify-between items-center p-2.5 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors"
                        >
                          <div className="min-w-0 pr-2">
                            <p className="font-black text-slate-900 truncate uppercase">
                              {st.name}
                            </p>
                            <p className="text-slate-400 font-medium truncate uppercase tracking-wider font-sans">
                              Active Store Registry
                            </p>
                          </div>
                          <div className="flex flex-col items-end shrink-0 text-right">
                            <p className="font-bold text-slate-600 uppercase">
                              Admin: {st.adminEmail || "None"}
                            </p>
                            <span className="text-[8px] font-black uppercase tracking-widest text-emerald-600 mt-1">
                              {st.status}
                            </span>
                          </div>
                        </div>
                  ...
                  <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                    <p className="font-extrabold text-[10px] text-slate-400 uppercase tracking-widest mb-4">
                      User Profiles Registries ({availableUsers.length})
                    </p>
                    <div className="max-h-60 overflow-y-auto space-y-2 pr-1 custom-scrollbar text-[10px]">
                      {availableUsers.map((usr, i) => {
                        const keyVal = usr.id || usr.username || `user-custom-${i}`;
                        if (usr.id === '' || usr.id === undefined || usr.id === null) {
                          console.warn("TEMPORARY KEY DETECTOR:", { file: "SuperAdmin.tsx", component: "UserProfilesRegistries", keyVal: usr.id, index: i });
                        }
                        return (
                          <div
                            key={keyVal}
                        ...
                        >
                          <div className="min-w-0 pr-2">
                            ...
                            <p className="text-slate-400 font-semibold truncate italic">
                              {usr.email}
                            </p>
                          </div>
                          <div className="flex flex-col items-end shrink-0 text-right">
                            <span className="px-2 py-0.5 bg-slate-900 text-white rounded font-mono text-[8px] font-black uppercase tracking-widest">
                              {usr.role}
                            </span>
